...
fn pick_init_bootstrap_helper_name(
    module_fns: &std::collections::HashMap<String, FnDecl>,
) -> Option<&'static str> {
    if module_fns.contains_key("bootstrap_init_for_testing") {
        Some("bootstrap_init_for_testing")
    } else if module_fns.contains_key("init_for_testing") {
        Some("init_for_testing")
    } else {
        None
    }
}
